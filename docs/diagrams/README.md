...
- `erd.mmd`: nguồn Mermaid có thể chỉnh sửa, gồm 20 bảng hiện tại và các quan hệ chính.
...
Thiết kế ngày 12/08/2026 bổ sung `sports`, `field_types`, các cột vị trí trên venue, snapshot tiền cọc trên booking và số liên hệ riêng tư trên yêu cầu ghép. Cập nhật bổ sung `match_messages` (chat theo kèo), `notifications` (Notification Center USER-only) và `media_images`, cùng cột `payments.checkout_url` cho VNPAY Sandbox. Kiến trúc runtime hiện hành dùng Leaflet/Nominatim theo ADR-036 và không tích hợp Google Maps API; ERD chỉ mô tả dữ liệu. Chỉ xuất lại `erd.png` từ đúng nguồn `erd.mmd` hiện hành.
