...
- `routes/`: trang chủ, xác thực, yêu cầu owner, quản lý/kiểm duyệt venue, field, pricing và health checks.
- `models/`: model `User`, `OwnerApplication`, `Venue`, `Field`, `FieldPriceSlot`, các vai trò và trạng thái.
- `forms/`: biểu mẫu Flask-WTF cho xác thực, owner application, venue, field và khung giá.
- `services/`: nghiệp vụ tài khoản, owner application, venue, field và pricing, bao gồm kiểm tra quyền sở hữu, chống chồng giờ, tính giá và transaction.
