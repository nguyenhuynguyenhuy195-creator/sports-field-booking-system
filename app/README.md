...
- `routes/`: trang chủ, xác thực, yêu cầu owner, quản lý/kiểm duyệt venue, quản lý field và health checks.
- `models/`: model `User`, `OwnerApplication`, `Venue`, `Field`, các vai trò và trạng thái.
- `forms/`: biểu mẫu Flask-WTF cho xác thực, owner application, venue và field.
- `services/`: nghiệp vụ tài khoản, owner application, venue và field, bao gồm kiểm tra quyền sở hữu và transaction.
