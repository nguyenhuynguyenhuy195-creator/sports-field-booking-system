...
- `routes/`: trang chủ, xác thực, yêu cầu owner, venue, field, pricing, maintenance, booking và health checks.
- `models/`: model `User`, `OwnerApplication`, `Venue`, `Field`, `FieldPriceSlot`, `FieldMaintenance`, `Booking`, `BookingPriceDetail`, các vai trò và trạng thái.
- `forms/`: biểu mẫu Flask-WTF cho xác thực, owner application, venue, field, khung giá, bảo trì và booking.
- `services/`: nghiệp vụ tài khoản, owner application, venue, field, pricing, maintenance và booking; bao gồm quyền sở hữu, khóa SQL Server, chống trùng lịch, snapshot giá và transaction.
